...
const mimetypesset string = `3D Crossword Plugin	application/vnd.hzn-3d-crossword	.x3d	IANA: 3D Crossword Plugin
...
func FindMimeTypeByExt(ext string, defaultExt string) (mimetype string, extfound string) {
	...
	if _, ok := mimeTypesCache[ext]; ok {
		mimetype = mimeTypesCache[ext][1]
		extfound = mimeTypesCache[ext][2]
	} else {
		scanner := bufio.NewScanner(strings.NewReader(mimetypesset))
		for scanner.Scan() {
			s := strings.Split(scanner.Text(), "\t")
			if s[2] == ext {
				mimetype = s[1]
				extfound = s[2]
				mimeTypesCache[ext] = s[:]
				break
			}
		}
	}
	return mimetype, extfound
}
